...
// Mark all queries of this student as resolved (used after admin updates record)
bool backend_markQueriesResolvedForRoll(int roll) {
    bool changed = false;
    for (auto &q : gQueries) {
        if (q.roll == roll) {
            q.status = "Resolved";
    ...
    return changed;   // true if at least one query was updated
}
...
HWND hAdminOutLabel, hQueryOutLabel;  // labels above outputs
...
// update student details (and mark queries resolved)
void GUI_UpdateStudent(HWND hwnd) {
    ...
    // also mark that student's queries as resolved
    bool hadQueries = backend_markQueriesResolvedForRoll(roll);

    if (hadQueries) {
        InfoBox(hwnd,
                "Student details updated.\r\n"
                "All related queries marked as Resolved.");
    } else {
    ...
    // refresh queries panel so you can see 'Resolved'
    string qout = backend_getAllQueries();
    SetWindowTextA(hStudOutput, qout.c_str());
}
